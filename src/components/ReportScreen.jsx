...
export default function ReportScreen({ answers, onReset, onNavigate, stepIndex }) {
  const [menuOpen, setMenuOpen] = useState(false);

  const orderedAnswers = STEPS.map(s => answers.find(a => a.step === s.id)).filter(Boolean);

  const rate = answers.find(a => a.step === "rate");
  const rhythm = answers.find(a => a.step === "rhythm");
  const abnormals = answers.filter(a => !a.ok);

  const impression = (() => {
    const parts = [];
    if (rhythm?.ok && rate?.ok) {
      parts.push("Normal sinus rhythm at a normal rate.");
    } else {
    ...
    if (others.length === 0) {
      parts.push("No additional significant abnormalities identified.");
    } else {
    ...
    <div style={{ background: "#060d14", minHeight: "100vh", color: "#e0f7e9", fontFamily: "'Courier New', monospace", maxWidth: 680, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px 8px", borderBottom: "1px solid #0d2a1a" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ background: "#0a3d20", color: "#00e676", padding: "3px 9px", borderRadius: 4, fontSize: 11, fontWeight: 800, letterSpacing: 1.5 }}>FINAL REPORT</span>
          <button
      ...
      <div style={{ padding: 14 }}>
        <div style={{ background: "#071409", border: "1px solid #00e676", borderRadius: 8, padding: 14, marginBottom: 20 }}>
          <div style={{ color: "#00e676", fontSize: 9.5, fontWeight: 800, letterSpacing: 2.5, marginBottom: 8 }}>IMPRESSION</div>
          <p style={{ color: "#c8f0d8", fontSize: 13, lineHeight: 1.68, margin: 0 }}>{impression}</p>
        </div>
        <div style={{ color: "#3a5a45", fontSize: 9.5, fontWeight: 800, letterSpacing: 2, marginBottom: 12 }}>SYSTEMATIC FINDINGS</div>
